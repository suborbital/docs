...
This can only be done as an [API call](../../customizing-functions/plugin-editor.md) from within your cluster. Since we’re currently not running an app in our cluster, we’ll just make the call from within!
...
The function editor is available through [building a specific URL](../../customize-plugins/plugin-editor). We can do that now that we have all the ingredients. In your browser, try opening up the following URL:
...
- `IDENT`: Customer identity, for example: `com.example.12345`
- `FUNCTION_NAME` : A name for your function
- `LANGUAGE_TEMPLATE`: [A template to be prefilled](../../customize-plugins/plugin-editor#configuration) when opening the editor for a new function, defaulting to `AssemblyScript`.

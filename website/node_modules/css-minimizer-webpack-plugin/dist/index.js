...
class CssMinimizerPlugin {
  ...
  constructor(options) {
    ...
    const {
      ...
      _utils.cssnanoMinify,
      minimizerOptions =
      /** @type {MinimizerOptions<InferDefaultType<T>>} */
      {},
      test = /\.css(\?.*)?$/i,
      warningsFilter = () => true,
      parallel = true,
    ...
    for (const asset of assetsForMinify) {
      scheduledTasks.push(async () => {
        ...
        if (!output) {
          ...
          try {
            result = await (getWorker ? getWorker().transform((0, _serializeJavascript.default)(options)) : (0, _minify.minify)(options));
          } catch (error) {
            ...
            /** @type {any} */
            error, name, hasSourceMap ? new _sourceMap.SourceMapConsumer(
            /** @type {RawSourceMap} */
          ...
          if (result.errors && result.errors.length > 0) {
            ...
            for (const error of result.errors) {
              output.warnings.push(CssMinimizerPlugin.buildError(error, name, hasSourceMap ? new _sourceMap.SourceMapConsumer(
              /** @type {RawSourceMap} */
              inputSourceMap) : // eslint-disable-next-line no-undefined
              undefined, // eslint-disable-next-line no-undefined
              hasSourceMap ? compilation.requestShortener : undefined));
          ...
          if (result.warnings && result.warnings.length > 0) {
            ...
            for (const warning of result.warnings) {
              const buildWarning = CssMinimizerPlugin.buildWarning(warning, name, this.options.warningsFilter, hasSourceMap ? new _sourceMap.SourceMapConsumer(
              /** @type {RawSourceMap} */
              inputSourceMap) : // eslint-disable-next-line no-undefined
              undefined, // eslint-disable-next-line no-undefined
              hasSourceMap ? compilation.requestShortener : undefined);
...
CssMinimizerPlugin.esbuildMinify = _utils.esbuildMinify;
